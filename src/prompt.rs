...
pub fn text(message: &str, opts: &TextPromptOptions) -> Result<String, crate::error::ConsolaError> {
...
) -> Result<bool, crate::error::ConsolaError> {
...
) -> Result<Vec<String>, crate::error::ConsolaError> {
...
mod backend {
    ...
    fn map_err_demand(e: std::io::Error) -> crate::error::ConsolaError {
        if e.kind() == std::io::ErrorKind::Interrupted {
            crate::error::ConsolaError::PromptCancelled
        } else {
            crate::error::ConsolaError::Prompt(e.to_string())
        }
    }
}
...
mod backend {
    ...
    pub(super) fn text(
        ...
        input
            .prompt()
            .map_err(|e| crate::error::ConsolaError::Prompt(e.to_string()))
    }
    ...
    pub(super) fn select(
        ...
        let sel = inquire::Select::new(message, labels.clone())
            .prompt()
            .map_err(|e| crate::error::ConsolaError::Prompt(e.to_string()))?;
        let i = labels.iter().position(|s| *s == sel).ok_or_else(|| {
            crate::error::ConsolaError::Lookup("selected label not found in options list".into())
        })?;
    ...
    pub(super) fn multiselect(
        ...
        selected
            ...
            .map(|sel| {
                let i = labels.iter().position(|s| *s == *sel).ok_or_else(|| {
                    crate::error::ConsolaError::Lookup(
                        "selected label not found in options list".into(),
                    )
                })?;
                Ok(opts.options[i].value.clone())
            })
...
mod backend {
    ...
    pub(super) fn text(
        _message: &str,
        _opts: &TextPromptOptions,
    ) -> Result<String, crate::error::ConsolaError> {
        Err(crate::error::ConsolaError::NoPromptBackend)
    }

    pub(super) fn confirm(
        _message: &str,
        _opts: &ConfirmPromptOptions,
    ) -> Result<bool, crate::error::ConsolaError> {
        Err(crate::error::ConsolaError::NoPromptBackend)
    }
