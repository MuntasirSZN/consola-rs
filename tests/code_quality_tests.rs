...
fn test_no_unsafe_in_main_library() {
    ...
    for line in lines {
        // Skip comments
        if line.contains("//") && line.find("unsafe").unwrap_or(usize::MAX) > line.find("//").unwrap_or(0) {
            continue;
        }
...
fn test_no_unwrap_expect_in_main_library() {
    ...
    for line in lines {
        ...
        if line.contains("//") {
            ...
            let unwrap_pos = line.find("unwrap()").or_else(|| line.find("expect(")).unwrap_or(usize::MAX);
    ...
    if !violations.is_empty() {
        eprintln!("Found {} unwrap()/expect() calls in main library code:", violations.len());
        for v in &violations {
            eprintln!("  {}", v);
        }
    }
